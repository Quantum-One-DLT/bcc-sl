...
-- The DSL and Bcc core have two different solutions to this:
--
-- * In the DSL a transaction can have an empty list of inputs
-- * In Bcc transaction lists transaction _hashes_ as their inputs, rather
...
--
-- In order to be able to compare the DSL or Bcc, we need to create an
-- initial "bootstrap" transaction that sets things up to mirror the Bcc
...
bootstrapTransaction ctxt@TransCtxt{..} = DSL.Transaction {
  ...
  where
    ...
    -- 'distributeBcc' matters. The original list is sorted by 'Address', which
    -- is basically random.
    ...
    totalBcc :: Word64
    totalBcc = sum $ map snd balances -- we're ignoring overflow here
