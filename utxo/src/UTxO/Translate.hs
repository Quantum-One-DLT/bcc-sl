...
module UTxO.Translate (
    -- * Monadic context for the translation from the DSL to Bcc
    TranslateT(..)
  , Translate
  , runTranslateT
...
{-------------------------------------------------------------------------------
  Testing infrastructure from bcc-sl-core
...
{-------------------------------------------------------------------------------
  ...
  dictionaries so that we can lift Bcc operations to the 'Translate' monad.
...
runTranslateT pm (TranslateT ta) =
    withProvidedMagicConfig pm $ \genesisConfig _ _ ->
      let nm = makeNetworkMagic pm
          env :: TranslateEnv
          env = TranslateEnv {
                    teContext = initContext nm (initBccContext genesisConfig)
                  , teUpdate  = Dict
                  }
...
translateGenesisHeader = view gbHeader <$> asks (ccBlock0 . tcBcc)
...
verifyBlocksPrefix era blocks txValRules =
    case splitEpochs blocks of
      ...
      ESRValid genEpoch (OldestFirst succEpochs) -> case era of
        SBFT _ -> validatedFromExceptT . throwError $ VerifyBlocksError "No support for SBFT validation in UTxO package."
        Original -> do
          BccContext{..} <- asks tcBcc
          let pm = ccMagic
