...
-- | A common interpretation for the UTxO DSL and abstract chain into Bcc
...
import           Universum
import           UTxO.Context (BccContext (..), TransCtxt (..))
...
weights = gdBootStakeholders . ccData . tcBcc
...
-- the stakes for the next epoch (in the Bcc codebase is referred to as a
...
createEpochBoundary ic = do
  ...
  where
    ...
    -- accurate reflection of how Bcc works.
