...
module Bcc.WalletClient (
    withdraw
  , randomAmount
  ) where

import           Bcc.Wallet.API.V1.Types (Payment (..), V1 (..))
...
-- | Computes the amount of BCC (units in entropic) to send in 'withdraw'
...
--
-- Simply sends a 'randomAmount' of BCC (units in entropic )to the supplied
-- 'Address'
...
-- See 'Bcc.Faucet.Init.processWithdrawals'
sendToQueue
