...
module Bcc.Faucet.Types.Config (
   FaucetConfig(..)
 , HasFaucetConfig(..)
...
import           Bcc.Wallet.API.V1.Types (AccountIndex, Payment,
                     PaymentSource (..), V1, WalletId (..))
import           Bcc.Wallet.Client (ClientError (..), WalletClient (..))
import           Pos.Core (Address (..))
import           Test.QuickCheck (Arbitrary (..), choose)
import           Test.QuickCheck.Arbitrary.Generic
import           Universum

import           Bcc.Faucet.Types.API
import           Bcc.Faucet.Types.Recaptcha
...
--
-- The amount of BCC (units in entropic) to send from the faucet is calculated
-- by
--
-- @
...
--------------------------------------------------------------------------------
-- | Config for the wallet used by the faucet as a source of BCC
data SourceWallet
...
data InitializedWallet = InitializedWallet {
  ...
  , _walletBalance       :: !Int64
    -- | Return address to send BCC back to
...
data FaucetEnv = FaucetEnv {
  ...
  , _feSourceWallet    :: !SourceWalletConfig
    -- | Return address for sending BCC back to the faucet
