...
import           Bcc.Wallet.API.V1.Types (Transaction (..), V1, unV1)
import           Pos.Core (Address (..))


import           Bcc.Faucet.Metrics
import           Bcc.Faucet.Types
import           Bcc.Faucet.Types.Recaptcha
import qualified Bcc.WalletClient as Client

-- | Top level type of the faucet API
type FaucetAPI = "withdraw" :> Summary "Requests BCC from the faucet"
                            :> ReqBody '[FormUrlEncoded, JSON] WithdrawalRequest
                            :> Post '[JSON] WithdrawalResult
            :<|> "return-address" :> Summary "Get the address to return BCC to"
                                  :> Get '[JSON] (V1 Address)

faucetServerAPI :: Proxy FaucetAPI
faucetServerAPI = Proxy

-- | Handler for the withdrawal of BCC from the faucet
withdraw :: (MonadFaucet c m) => WithdrawalRequest -> m WithdrawalResult
withdraw wr = withSublogger (LoggerName "withdraw") $ do
    logInfo "Attempting to send BCC"
...
-- | Get the address to return BCC to
