...
import           Pos.Chain.Update (ApplicationName (ApplicationName),
                     ...
                     ccLastKnownBlockVersion_L, sbftEraFlagValue)
...
mutateConfigurationForSbft :: Configuration -> Configuration
mutateConfigurationForSbft = over ccGenesis_L (updateUnlockStakeEpoch sbftEraFlagValue)
 where
...
testHardForkMechanism =
 ...
 where
  ...
  mutateConf = mutateConfigurationForSbft
  expectedResult :: ExpectedResult
  expectedResult = SuccessFullUpdate
  blockVersion :: BlockVersion
  blockVersion = BlockVersion 1 0 0
  softwareVersion :: SoftwareVersion
  softwareVersion = SoftwareVersion (ApplicationName "bcc-sl") 0
  blockVersionModifier :: BlockVersionModifier
  blockVersionModifier = def { bvmUnlockStakeEpoch = Just sbftEraFlagValue }
  checkBvd :: BlockVersionData -> Bool
  checkBvd bvd = bvdUnlockStakeEpoch bvd == sbftEraFlagValue
  errMsg :: String
...
test4 targetblocksize expectedResult =
 ...
 where
  ...
  softwareVersion :: SoftwareVersion
  softwareVersion = SoftwareVersion (ApplicationName "bcc-sl") 1
  blockVersionModifier :: BlockVersionModifier
