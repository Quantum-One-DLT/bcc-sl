...
import           Bcc.Cluster (NodeType (..), mkNamedNodes)
import           Bcc.Cluster.Environment (Artifact, initializeArtifact,
                     prepareEnvironment, withStateDirectory)
import           Bcc.Cluster.Util (stripFilterPrefix, unsafeIntFromString)


-- | Command-Line Interface specification. See http://docopt.org/
cli :: Docopt
cli = [docopt|
bcc-sl-prepare-environment

Generate a default environment for a given cluster configuration

Usage:
  bcc-sl-prepare-environment <prefix> [options]
  bcc-sl-prepare-environment --help
