...
# bcc-keygen.  It creates an new configuration.yaml with the
...
let
  ...
  genesisTools = [ yaml2json jq python3 coreutils gnused bcc-sl-tools ];
  configSource = bcc-sl.src + "/configuration.yaml";
...
in
  writeScript "prepare-genesis" ''
    ...
    bcc-keygen --system-start $(cat system-start.txt) \
