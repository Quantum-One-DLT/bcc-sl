...
# USAGE
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#   scripts/build/bcc-sl.sh                 build all packages one-by-one
#   scripts/build/bcc-sl.sh -t              build and run tests
#   scripts/build/bcc-sl.sh core|db|...     build only a specific project (see below)
#   scripts/build/bcc-sl.sh -k              typecheck but do not build
#   scripts/build/bcc-sl.sh -c              do stack clean
#
# Consider symlinking the script as `b` into the bcc-sl folder because
# typing `scripts/build/bcc-sl.sh` is annoying.

# PROJECTS
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#   Project argument                Package name
#   :
#   core, db, etc.                  bcc-sl-{core,db,etc.}
#   lib                             bcc-sl
...
projects="networking binary util crypto core db chain infra lib node client generator auxx tools explorer wallet"
...
do
  ...
  # benchmarks config
    ...
    # We want:
    # • --flag bcc-sl-core:-asserts ($asserts)
...
if [[ "$for_installer" == true ]]; then
  commonargs="$commonargs --flag bcc-sl-tools:for-installer"
fi

if [[ "$asserts" == false ]]; then
  commonargs="$commonargs --flag bcc-sl-core:-asserts"
fi
...
if [[ "$spec_prj" == "" ]]; then
  ...
  to_build="bcc-sl-node bcc-wallet"
elif [[ "$spec_prj" == "explorer" ]]; then
  to_build="bcc-sl-node bcc-sl-explorer"
