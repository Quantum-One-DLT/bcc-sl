...
blacklistedPackages = [ "bcc-sl-auxx"
                      , "bcc-sl-tools"
                      ]

--------------------------------------------------------------------------------
-- Filter `bcc-sl-auxx` & `bcc-sl-tools` as they cannot be
...
unavoidableDeps myself x = and [
      x /= myself
    , not ("bcc" `T.isInfixOf` (fst x))
    ]

--------------------------------------------------------------------------------
-- | Filter "unavoilable" dependencies like the ones of the bcc family.
...
main = do
    ...
    let tableHeader         =  printf "%-40s" ("Package" :: String)
                            ...
                            <> printf "%-70s" ("Reverse deps (excluding bcc-*)"  :: String)
...
showRevDeps []  = T.pack $ printf "%-4d%s" (0 :: Int) ("(possibly bcc depends on it)" :: String)
