...
-- versions of an API. Think about 'Address' or 'Coin'. Those are core Bcc types we want to
...
data TxFeePolicy = TxFeePolicy
    { tfpA :: MeasuredIn 'EntropicPerByte Core.Coeff
    , tfpB :: MeasuredIn 'Entropic Core.Coeff
...
instance ToSchema TxFeePolicy where
    declareNamedSchema _ = do
        ...
        -- fix this ATM ¯\_(ツ)_/¯
        NamedSchema _ a <- declareNamedSchema $ Proxy @(MeasuredIn 'EntropicPerByte Double)
        NamedSchema _ b <- declareNamedSchema $ Proxy @(MeasuredIn 'Entropic Double)
