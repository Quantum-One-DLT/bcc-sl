...
import           Pos.DB.Lrc.SBFT (getSlotLeaderSbft)
...
blockCreator genesisConfig txpConfig slotId diffusion = do
    ...
    case era of
        ...
        SBFT _   -> do
            tipHeader <- DB.getTipHeader
            whenEpochBoundarySbft (siEpoch slotId) tipHeader (\ei -> do
                logDebug $ "blockCreator SBFT: running lrcSingleShot"
                lrcSingleShot genesisConfig ei)
            dropSbftEbb genesisConfig txpConfig
            blockCreatorSbft genesisConfig txpConfig slotId diffusion
  where
    whenEpochBoundarySbft ::
        ( Applicative m
        )
        => EpochIndex
        -> BlockHeader
        -> (EpochIndex -> m ())
        -> m ()
    whenEpochBoundarySbft currentEpoch tipHeader actn = do
        case tipHeader of
...
blockCreatorSbft
    ...
    -> SlotId
    -> Diffusion m -> m ()
blockCreatorSbft genesisConfig txpConfig (slotId@SlotId {..}) diffusion = do
    (leader, pske) <- getSlotLeaderSbft genesisConfig slotId
...
dropSbftEbb ::
    ...
    -> TxpConfiguration
    -> m ()
dropSbftEbb genesisConfig txpConfig = do
    -- not sure if everything needs to run inside StateLock
    tipHeader <- DB.getTipHeader
    case tipHeader of
        BlockHeaderMain _      -> pure ()
        -- If we're starting up a chain in SBFT mode, we need to ensure
        ...
        BlockHeaderGenesis _   -> unless (tipHeader ^. epochIndexL == 0) $ do
            ...
            case mbEbbBlund of
                Nothing -> Prelude.error "Pos.Worker.Block.dropSbftEbb: unable to get blund for EBB"
                Just ebbBlund -> do
...
blockCreatorOriginal genesisConfig txpConfig (slotId@SlotId {..}) diffusion = do
    ...
    case era of
        ...
        -- being run, then we want to switch over to `blockCreatorSbft`, while
        -- running the logic which precedes its call in `blockCreator`.
        -- So we just re-enter `blockCreator`.
        SBFT _ -> blockCreator genesisConfig txpConfig slotId diffusion
