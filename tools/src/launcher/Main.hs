...
getLauncherOptions = do
    ...
    klarityDir <- takeDirectory <$> getExecutablePath
    case Sys.os of
      "mingw32" -> do
        -- This is used by 'substituteEnvVars', later
        setEnv "KLARITY_DIR" klarityDir
      _ -> pure ()
    -- linux and windows use KLARITY_DIR for different things, but having a single var with the same meaning will help with some issues
    setEnv "KLARITY_INSTALL_DIRECTORY" klarityDir
    configPath <- maybe defaultConfigPath pure maybeConfigPath

    -- [CSL-2503] remove once bcc-node is capable of finding the file on its own and klarity no longer needs it
  ...
  where
    ...
    programInfo = info (helper <*> versionOption <*> launcherArgsParser) $
        fullDesc <> progDesc ""
                 <> header "Tool to launch Bcc SL."
                 <> footerDoc usageExample

    versionOption = infoOption
        ("bcc-launcher-" <> showVersion version)
...
Command example:
  ...
  stack exec -- bcc-launcher --config launcher-config.yaml
...
serverScenario pm ndbp node updater lo = do
    ...
    if exitCode == ExitFailure 20 then do
        ...
        whenJust (loReportServer lo) $ \repServ -> do
            logInfo $ sformat ("Sending logs to "%stext) (toText repServ)
            reportNodeCrash pm exitCode (loLogsPrefix lo) (loNodeLogConfig lo) repServ
...
writeWindowsUpdaterRunner runnerPath = liftIO $ do
    ...
    writeFile (toString runnerPath) $ unlines
        ...
        , "del %1"
        -- Run launcher again
        , "start \"bcc launcher\" /b " <> (quote $ toText exePath) <> " " <> (unwords $ map (quote . toText) launcherArgs)
        -- Delete the bat file
        , "(goto) 2>nul & del \"%~f0\""
        ]
...
spawnNode nd doesWalletLogToConsole = do
    ...
    cr <- liftIO $ case mLogPath of
        Just lp -> do
            ...
            -- TODO (jmitchell): Find a safe, reliable way to print `logPath`. Bcc
