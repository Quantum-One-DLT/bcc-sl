...
getUpdateGenOptions = execParser programInfo
  where
    programInfo = info (helper <*> versionOption <*> optionsParser) $
        fullDesc <> progDesc ("")
                 <> header "Bcc SL updates generator."
                 <> footerDoc usageExample

    versionOption = infoOption
        ("bcc-genupdate-" <> showVersion version)
...
Command example:
  ...
  stack exec -- bcc-genupdate --old /tmp/app-v000 --new /tmp/app-v001 -o /tmp/app-update.tar

Both directories must have equal file structure (e.g. they must contain the same
files in the same subdirectories correspondingly), otherwise 'bcc-genupdate' will fail.

Please note that 'bcc-genupdate' uses 'bsdiff' program, so make sure 'bsdiff' is available in the PATH.|]
