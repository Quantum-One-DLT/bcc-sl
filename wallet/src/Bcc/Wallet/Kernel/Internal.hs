...
-- Bcc specific types (except those types that appear in the translation
-- of the UTxO DSL), so that we can test it outside of a node context also
-- (in unit tests).
module Bcc.Wallet.Kernel.Internal (
    -- * Passive wallet
    PassiveWallet(..)
    -- ** Lenses
  , walletKeystore
...
import           Bcc.Wallet.API.Types.UnitOfMeasure (MeasuredIn (..),
                     UnitOfMeasure (..))
import           Bcc.Wallet.Kernel.DB.AcidState (DB)
...
import           Bcc.Wallet.Kernel.Submission (WalletSubmission)
