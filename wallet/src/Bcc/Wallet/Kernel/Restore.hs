...
module Bcc.Wallet.Kernel.Restore
    ( restoreWallet
    , restoreKnownWallet
    , continueRestoration
...
import           Bcc.Wallet.Kernel (walletLogMessage)
import qualified Bcc.Wallet.Kernel as Kernel
import           Bcc.Wallet.Kernel.DB.AcidState (ApplyHistoricalBlocks (..),
...
import           Bcc.Wallet.Kernel.DB.HdWallet.Create (CreateHdRootError)
import           Bcc.Wallet.Kernel.DB.InDb (fromDb)
import           Bcc.Wallet.Kernel.DB.Resolved (ResolvedBlock)
import qualified Bcc.Wallet.Kernel.DB.Spec.Update as Spec
import           Bcc.Wallet.Kernel.DB.TxMeta.Types
import           Bcc.Wallet.Kernel.Decrypt (decryptAddress,
                     eskToWalletDecrCredentials)
import           Bcc.Wallet.Kernel.Internal (WalletRestorationInfo (..),
...
import           Bcc.Wallet.Kernel.NodeStateAdaptor (Lock, LockContext (..),
                     ...
                     withNodeState)
import           Bcc.Wallet.Kernel.PrefilterTx (AddrWithId,
                     PrefilteredBlock, UtxoWithAddrId, WalletKey,
                     prefilterBlock, prefilterUtxo', toHdAddressId,
                     toPrefilteredUtxo)
import           Bcc.Wallet.Kernel.Read (foreignPendingByAccount,
                     getWalletSnapshot)
...
import           Bcc.Wallet.Kernel.Util.Core (utxoBalance)
import           Bcc.Wallet.Kernel.Wallets (createWalletHdRnd)
...
restoreWallet pw hasSpendingPassword defaultBccAddress name assurance esk = do
    ...
    case walletInitInfo of
      WalletCreate utxos -> do
        root <- createWalletHdRnd pw hasSpendingPassword defaultBccAddress name assurance esk $
      ...
      WalletRestore utxos tgt -> do
          ...
          -- already exists.
          mRoot <- createWalletHdRnd pw hasSpendingPassword defaultBccAddress name assurance esk $
                  \root defaultHdAccount defaultHdAddress ->
...
{-------------------------------------------------------------------------------
  Shared with Bcc.Wallet.WalletLayer.Kernel.Wallets
-------------------------------------------------------------------------------}
