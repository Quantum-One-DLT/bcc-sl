...
module Bcc.Wallet.Kernel.BListener (
...
import           Bcc.Wallet.Kernel.DB.AcidState (ApplyBlock (..),
...
import           Bcc.Wallet.Kernel.DB.Resolved (ResolvedBlock, rbContext)
import           Bcc.Wallet.Kernel.DB.Spec.Pending (Pending)
import           Bcc.Wallet.Kernel.DB.Spec.Update (ApplyBlockFailed (..))
...
import           Bcc.Wallet.WalletLayer.Kernel.Wallets
...
--
-- NOTE: The Shardagnostic protocol says that this is only valid if the number of
