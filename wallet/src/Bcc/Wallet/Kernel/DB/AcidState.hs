...
module Bcc.Wallet.Kernel.DB.AcidState (
    -- * Top-level database
    DB(..)
  , dbHdWallets
  , defDB
...
import qualified Bcc.Wallet.Kernel.DB.HdWallet.Create as HD
import qualified Bcc.Wallet.Kernel.DB.HdWallet.Delete as HD
import qualified Bcc.Wallet.Kernel.DB.HdWallet.Update as HD
import           Bcc.Wallet.Kernel.DB.InDb (InDb (InDb), fromDb)
...
import           Bcc.Wallet.Kernel.DB.Updates (Updates, noUpdates)
import qualified Bcc.Wallet.Kernel.DB.Updates as Updates
...
import           Bcc.Wallet.Kernel.Util.NonEmptyMap (NonEmptyMap)
import qualified Bcc.Wallet.Kernel.Util.NonEmptyMap as NEM
...
--  * V1 API defined in "Bcc.Wallet.API.V1.*" (in @src/@)
data DB = DB {
...
-- 'createHdWallet'/'createWalletHdRnd' in "Bcc.Wallet.Kernel.Wallets".
--
