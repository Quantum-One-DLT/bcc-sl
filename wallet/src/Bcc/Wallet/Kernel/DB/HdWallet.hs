...
-- | HD wallets
module Bcc.Wallet.Kernel.DB.HdWallet (
    -- * Supporting types
    WalletName(..)
  , AccountName(..)
  , HdAccountIx(..)
...
import           Bcc.Wallet.API.V1.Types (V1 (..), WAddressMeta(WAddressMeta))
import           Bcc.Wallet.Kernel.DB.BlockContext
import           Bcc.Wallet.Kernel.DB.InDb
import           Bcc.Wallet.Kernel.DB.Spec
import           Bcc.Wallet.Kernel.DB.Util.AcidState
import           Bcc.Wallet.Kernel.DB.Util.IxSet hiding (foldl')
import qualified Bcc.Wallet.Kernel.DB.Util.IxSet as IxSet hiding (Indexable)
import qualified Bcc.Wallet.Kernel.DB.Util.Zoomable as Z
import           Bcc.Wallet.Kernel.Decrypt (WalletDecrCredentials, decryptAddress)
import           Bcc.Wallet.Kernel.NodeStateAdaptor (SecurityParameter (..))
import qualified Bcc.Wallet.Kernel.Util.StrictList as SL
import           Bcc.Wallet.Kernel.Util.StrictNonEmpty (StrictNonEmpty (..))
import qualified Bcc.Wallet.Kernel.Util.StrictNonEmpty as SNE
...
-- are taken from <https://bccdocs.com/bcc/transaction-assurance/>
...
data UnknownHdAddress =
  ...
  | UnknownHdBccAddress Core.Address
...
-- | Zoom to the specified Bcc address
--
...
zoomHdBccAddress :: forall e a.
                        (UnknownHdAddress -> e)
                     -> Core.Address
                     -> Query' e HdAddress a -> Query' e HdWallets a
zoomHdBccAddress embedErr addr =
  ...
  where
    ...
    err = missing $ embedErr (UnknownHdBccAddress addr)
...
instance Buildable UnknownHdAddress where
    ...
    build (UnknownHdBccAddress coreAddress) =
        bprint ("UnknownHdBccAddress " % build) coreAddress
