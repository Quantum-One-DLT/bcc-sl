module Bcc.Wallet.Kernel.DB.Read (
...
import           Bcc.Wallet.Kernel.DB.AcidState (DB, dbHdWallets)
import           Bcc.Wallet.Kernel.DB.BlockMeta (AddressMeta)
import           Bcc.Wallet.Kernel.DB.HdWallet
import qualified Bcc.Wallet.Kernel.DB.HdWallet.Read as HD
...
{-------------------------------------------------------------------------------
  Lift functions from "Bcc.Wallet.Kernel.DB.Spec.Read"
  ...
  ("Bcc.Wallet.Kernel.DB.SomeWalletType.Read") should then be unified
...
lookupBccAddress :: DB -> Address -> Either UnknownHdAddress HdAddress
lookupBccAddress = liftHd1 HD.lookupBccAddress
