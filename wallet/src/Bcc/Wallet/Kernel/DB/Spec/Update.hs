...
module Bcc.Wallet.Kernel.DB.Spec.Update (
    -- * Errors
    NewPendingFailed(..)
  , NewForeignFailed(..)
  , ApplyBlockFailed(..)
...
import qualified Bcc.Wallet.Kernel.Util.Core as Core
import qualified Bcc.Wallet.Kernel.Util.Strict as Strict
...
-- an empty set, thus avoiding the extra dependency on @bcc-sl-core-test@.
