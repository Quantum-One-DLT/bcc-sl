...
module Bcc.Wallet.Kernel.DB.HdWallet.Read (
    -- | Summarize
    accountsByRootId
  , addressesByRootId
...
import           Bcc.Wallet.Kernel.DB.Spec (IsCheckpoint (..),
                     cpAddressMeta)
...
lookupBccAddress :: Address -> Query' UnknownHdAddress HdWallets HdAddress
lookupBccAddress addr = zoomHdBccAddress identity addr $ ask
