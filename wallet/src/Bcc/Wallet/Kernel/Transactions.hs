...
{-# LANGUAGE RankNTypes                 #-}
module Bcc.Wallet.Kernel.Transactions (
      pay
    , estimateFees
    , redeemBcc
    -- * Errors
...
import           Bcc.Crypto.Wallet (DerivationIndex)
...
import           Bcc.Wallet.Kernel.CoinSelection.FromGeneric
                     ...
                     estimateBccFee, estimateMaxTxInputs)
import qualified Bcc.Wallet.Kernel.CoinSelection.FromGeneric as CoinSelection
import           Bcc.Wallet.Kernel.CoinSelection.Generic
                     (CoinSelHardErr (..))
import           Bcc.Wallet.Kernel.DB.AcidState (DB, NewForeignError,
                     NewPendingError)
...
import           Bcc.Wallet.Kernel.Internal (ActiveWallet (..),
                     PassiveWallet (..), walletNode)
import qualified Bcc.Wallet.Kernel.Internal as Internal
import qualified Bcc.Wallet.Kernel.Keystore as Keystore
import qualified Bcc.Wallet.Kernel.NodeStateAdaptor as Node
import           Bcc.Wallet.Kernel.Pending (PartialTxMeta, newForeign,
                     newPending)
import           Bcc.Wallet.Kernel.Read (getWalletSnapshot)
import           Bcc.Wallet.Kernel.Types (AccountId (..),
                     RawResolvedTx (..), WalletId (..))
import           Bcc.Wallet.Kernel.Util.Core
import           Bcc.Wallet.WalletLayer.Kernel.Conv (exceptT)
...
prepareUnsignedTxWithSources activeWallet opts srcAccountId payees spendingPassword = runExceptT $ do
  ...
  where
    ...
    prepareSourceAddress db addr
        = case lookupBccAddress db address of
    ...
    retrying retryPolicy shouldRetry $ \rs -> do
        res <- runExceptT $ do
            ...
            firstSrcHdAddress <- withExceptT PaymentNoHdAddressForSrcAddress $ exceptT $
                lookupBccAddress snapshot firstSrcAddress
...
mkSigner nm spendingPassword (Just esk) snapshot addr =
    case Getters.lookupBccAddress snapshot addr of
...
-- | An estimate of the Tx fees in Bcc based on a sensible number of defaults.
bccFee :: TxFeePolicy -> Int -> NonEmpty Coin -> Coin
bccFee (TxFeePolicyTxSizeLinear policy) inputs outputs =
    Core.mkCoin $
      estimateBccFee policy inputs (toList $ fmap Core.getCoin outputs)
bccFee TxFeePolicyUnknown{} _ _ =
    error "bccFee: unknown policy"
...
data RedeemBccError =
    ...
    -- | We failed to translate the redeem key to a valid Bcc address
  | RedeemBccErrorCreateAddressFailed Kernel.CreateAddressError
  ...
  | RedeemBccMultipleOutputs Address

    -- | We were unable to submit the transaction
    --
    -- If this error happens, it almost certainly indicates a bug.
  | RedeemBccNewForeignFailed NewForeignError

instance Buildable RedeemBccError where
    build (RedeemBccUnknownAccountId err) =
      bprint ("RedeemBccUnknownAccountId " % build) err
    build (RedeemBccErrorCreateAddressFailed err) =
      bprint ("RedeemBccErrorCreateAddressFailed " % build) err
    build (RedeemBccNotAvailable addr) =
      bprint ("RedeemBccNotAvailable " % build) addr
    build (RedeemBccMultipleOutputs addr) =
      bprint ("RedeemBccMultipleOutputs " % build) addr
    build (RedeemBccNewForeignFailed err) =
      bprint ("RedeemBccNewForeignFailed " % build) err

-- | Redeem Bcc voucher
--
-- NOTE: The account must already exist, it is /not/ created implicitly if it
-- does not yet exist.
redeemBcc :: ActiveWallet
          -> HdAccountId      -- ^ Account ID
          -> PassPhrase       -- ^ Spending password
          -> RedeemSecretKey  -- ^ Redemption key
          -> IO (Either RedeemBccError (Tx, TxMeta))
redeemBcc w@ActiveWallet{..} accId pw rsk = runExceptT $ do
    let pm = walletPassive ^. Internal.walletProtocolMagic
    snapshot   <- liftIO $ getWalletSnapshot walletPassive
    _accExists <- withExceptT RedeemBccUnknownAccountId $ exceptT $
                    lookupHdAccountId snapshot accId
    changeAddr <- withExceptT RedeemBccErrorCreateAddressFailed $ ExceptT $ liftIO $
                    Kernel.createAddress
                      pw
                      (AccountIdHdRnd accId)
                      walletPassive
    (tx, meta) <- mkTx pm changeAddr
    withExceptT RedeemBccNewForeignFailed $ ExceptT $ liftIO $
      newForeign
        w
        accId
        tx
  ...
  where
    ...
    --   to compute the createdAt timestamp for `TxMeta`
    mkTx :: ProtocolMagic -> Address -> ExceptT RedeemBccError IO (TxAux, TxMeta)
    mkTx pm output = do
        ...
        (inp@(TxInUtxo inHash inIx), coin) <-
          case utxo of
            [i]   -> return i
            []    -> throwError $ RedeemBccNotAvailable    (redeemAddr nm)
            _:_:_ -> throwError $ RedeemBccMultipleOutputs (redeemAddr nm)
