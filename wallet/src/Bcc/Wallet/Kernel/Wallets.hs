module Bcc.Wallet.Kernel.Wallets (
      createHdWallet
    , updateHdWallet
    , updatePassword
    , deleteHdWallet
...
import           Bcc.Wallet.Kernel.DB.AcidState (CreateHdWallet (..),
...
import           Bcc.Wallet.Kernel.Decrypt (eskToWalletDecrCredentials)
...
createHdWallet pw mnemonic spendingPassword assuranceLevel walletName = do
    ...
    -- STEP 2.5: Generate the fresh Bcc Address which will be used for the
    -- companion 'HdAddress'
    let mbHdAddress =
            newHdAddress nm
                         esk
...
createWalletHdRnd pw hasSpendingPassword defaultBccAddress name assuranceLevel esk createWallet = do
...
-- | Given a Bcc 'Address', it returns a default 'HdAddress' at a fixed
...
defaultHdAddressWith :: HDPassphrase
                     ...
                     -> Address
                     -> Maybe HdAddress
defaultHdAddressWith hdPass rootId bccAddress = do
    (hdAccountIx, hdAddressIx) <- HD.decryptHdLvl2DerivationPath hdPass bccAddress
    let hdAccountId = HdAccountId rootId hdAccountIx
        hdAddressId = HdAddressId hdAccountId hdAddressIx
    pure $ HD.HdAddress hdAddressId (InDb bccAddress)
