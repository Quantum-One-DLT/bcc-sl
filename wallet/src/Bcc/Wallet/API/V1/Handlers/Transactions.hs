...
module Bcc.Wallet.API.V1.Handlers.Transactions (
      handlers
    , newTransaction
...
import           Bcc.Wallet.Kernel.CoinSelection.FromGeneric
                     (ExpenseRegulation (..))
import           Bcc.Wallet.Kernel.DB.HdWallet (UnknownHdAccount)
...
import           Bcc.Wallet.WalletLayer.Kernel.Conv (toInputGrouping)
...
redeemBcc :: ActiveWalletLayer IO
          -> Redemption
          -> Handler (APIResponse Transaction)
redeemBcc aw redemption = liftIO $ do
    res <- WalletLayer.redeemBcc aw redemption
    case res of
      Left err        -> throwM err
      Right (_, meta) -> txFromMeta aw embedErr meta
  where
    embedErr :: UnknownHdAccount -> WalletLayer.RedeemBccError
    embedErr = WalletLayer.RedeemBccError . Kernel.RedeemBccUnknownAccountId
