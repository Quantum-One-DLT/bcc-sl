...
module Bcc.Wallet.API.V1.ReifyWalletError (
...
import           Bcc.Wallet.Kernel.CoinSelection.Generic
                     (CoinSelHardErr (..))
import           Bcc.Wallet.Kernel.DB.AcidState (NewForeignError (..),
                     NewPendingError (..))
...
translateWalletLayerErrors ex = do
       (    try' @CreateAddressError createAddressError
        ...
        <|> try' @RedeemBccError redeemBccError
...
noHdAddressForSrcAddress e = case e of
    ...
    (HD.UnknownHdBccAddress _coreAddr) ->
        V1.AddressNotFound
...
redeemBccError :: RedeemBccError -> V1.WalletError
redeemBccError e = case e of
    (RedeemBccError e') -> case e' of
        (Kernel.RedeemBccUnknownAccountId e'') ->
            unknownHdAccount e''

        (Kernel.RedeemBccErrorCreateAddressFailed e'') ->
            createAddressErrorKernel e''

        (Kernel.RedeemBccNotAvailable _addr) ->
            V1.TxRedemptionDepleted

        ex@(Kernel.RedeemBccMultipleOutputs _addr) ->
            V1.UnknownError $ (sformat build ex)

        Kernel.RedeemBccNewForeignFailed e'' ->
            newForeignError e''

    (RedeemBccWalletIdDecodingFailed _txt) ->
            V1.WalletNotFound

    (RedeemBccInvalidRedemptionCode _) ->
        V1.TxRedemptionDepleted
