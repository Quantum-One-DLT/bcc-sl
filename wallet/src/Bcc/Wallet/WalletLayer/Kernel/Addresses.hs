...
module Bcc.Wallet.WalletLayer.Kernel.Addresses (
    createAddress
  , getAddresses
  , validateAddress
  , importAddresses
...
import           Bcc.Wallet.API.Response (SliceOf (..))
...
import           Bcc.Wallet.WalletLayer (CreateAddressError (..),
...
validateAddress rawText db = runExcept $ do
    bccAddress <- withExceptT (\_err -> ValidateAddressDecodingFailed rawText) $
                        exceptT $ decodeTextAddress rawText
    let mAddr = runExcept $ do
          addr <- exceptT $
                    Kernel.lookupBccAddress db
                      bccAddress
