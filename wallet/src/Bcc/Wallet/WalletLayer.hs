...
import           Bcc.Wallet.API.Request.Filter (FilterOperations (..))
import           Bcc.Wallet.API.Request.Sort (SortOperations (..))
import           Bcc.Wallet.API.Response (APIResponse, SliceOf (..))
import           Bcc.Wallet.API.V1.Types (Account, AccountBalance,
                     ...
                     WalletUpdate)
import qualified Bcc.Wallet.Kernel.Accounts as Kernel
...
import qualified Bcc.Wallet.Kernel.DB.HdWallet as Kernel
...
import           Bcc.Wallet.WalletLayer.ExecutionTimeLimit
                     (TimeExecutionLimit)
import           Bcc.Wallet.WalletLayer.Kernel.Conv (InvalidRedemptionCode)
...
data ValidateAddressError =
    ...
    -- ^ When trying to decode this raw 'Text' into a proper Bcc
...
data ActiveWalletLayer m = ActiveWalletLayer {
    ...
    , estimateFees :: InputGrouping
      ...
      -- | Redeem bcc
    , redeemBcc :: Redemption -> m (Either RedeemBccError (Tx, TxMeta))
...
data RedeemBccError =
    RedeemBccError Kernel.RedeemBccError
  | RedeemBccWalletIdDecodingFailed Text
  | RedeemBccInvalidRedemptionCode InvalidRedemptionCode

instance Show RedeemBccError where
    show = formatToString build

instance Exception RedeemBccError

instance Buildable RedeemBccError where
    build (RedeemBccError err) =
        bprint ("RedeemBccError " % build) err
    build (RedeemBccWalletIdDecodingFailed txt) =
        bprint ("RedeemBccWalletIdDecodingFailed " % build) txt
    build (RedeemBccInvalidRedemptionCode txt) =
        bprint ("RedeemBccInvalidRedemptionCode " % build) txt
