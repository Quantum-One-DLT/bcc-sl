...
-- | Interpreter from the DSL to Bcc types
module UTxO.ToBcc.Interpreter (
...
import           Bcc.Wallet.Kernel.DB.BlockMeta (AddressMeta,
                     BlockMeta (..))
import           Bcc.Wallet.Kernel.DB.InDb
import           Bcc.Wallet.Kernel.DB.Resolved
import           Bcc.Wallet.Kernel.Types
...
--
-- NOTE: In Bcc there is no equivalent of the boot transaction and hence
...
initIntCtxt boot = do
    ...
    leaders     <- asks (ccInitLeaders . tcBcc)
    initStakes  <- asks (ccStakes      . tcBcc)
    return $ IntCtxt {
          _icTxMeta = Map.singleton (DSL.hash boot) TxMeta {
               tmTx   = boot
             , tmHash = error "initIntCtxt: bootstrap transaction has no Bcc hash"
             }
...
-- | Lookup the Bcc hash for the given DSL hash
...
{-------------------------------------------------------------------------------
  Translate the DSL UTxO definitions to Bcc types

  NOTE: Delegation in Bcc is described in the document
  "Delegation and Stake Locking in Bcc SL"
  <bcc-sl-articles/delegation/pdf/article.pdf>.
-------------------------------------------------------------------------------}

data DSL2Bcc

instance Interpretation DSL2Bcc where
  type IntCtx DSL2Bcc = IntT

{-------------------------------------------------------------------------------
  Instances that read, but not update, the state
-------------------------------------------------------------------------------}

instance Interpret DSL2Bcc h DSL.Value where
  type Interpreted DSL2Bcc DSL.Value = Coin

  int :: Monad m => DSL.Value -> IntT h e m Coin
  int = return . mkCoin

instance Interpret DSL2Bcc h Addr where
  type Interpreted DSL2Bcc Addr = AddrInfo

  int :: Monad m => Addr -> IntT h e m AddrInfo
  int = asks . resolveAddr

instance DSL.Hash h Addr => Interpret DSL2Bcc h (DSL.Input h Addr) where
  type Interpreted DSL2Bcc (DSL.Input h Addr) = (TxOwnedInput SomeKeyPair, ResolvedInput)
  ...
  int inp@DSL.Input{..} = do
      ...
      resolvedInput <- (int @DSL2Bcc) spentOutput
      isBootstrap   <- isBootstrapTransaction <$> findHash' inpTrans

      if isBootstrap
        then do
          AddrInfo{..} <- (int @DSL2Bcc) $ DSL.outAddr spentOutput
          -- See explanation at 'bootstrapTransaction'
          return (
                   (addrInfoAddrKey, TxInUtxo (unsafeHash addrInfoBcc) 0)
                 , resolvedInput
                 )
        else do
          AddrInfo{..} <- (int @DSL2Bcc) $ DSL.outAddr spentOutput
...
instance Interpret DSL2Bcc h (DSL.Output h Addr) where
  type Interpreted DSL2Bcc (DSL.Output h Addr) = TxOutAux

  int :: Monad m
      => DSL.Output h Addr -> IntT h e m TxOutAux
  int DSL.Output{..} = do
      AddrInfo{..} <- (int @DSL2Bcc) outAddr
      outVal'      <- (int @DSL2Bcc) outVal
      let addrAttrSize = biSize (addrAttributes addrInfoBcc)
      unless (addrAttrSize <= boundAddrAttrSize) $
        throwError $ Left $ IntAddrAttrBoundsExceeded addrAttrSize

      return TxOutAux {
          toaOut = TxOut {
              txOutAddress = addrInfoBcc
            , txOutValue   = outVal'
            }
        }

instance DSL.Hash h Addr => Interpret DSL2Bcc h (DSL.Utxo h Addr) where
  type Interpreted DSL2Bcc (DSL.Utxo h Addr) = Utxo
...
{-------------------------------------------------------------------------------
  ...
  Block metadata: defines a DSL `BlockMeta'` analogy to the Bcc `BlockMeta`
  along with an interpreter instance to translate `BlockMeta'` to `BlockMeta`
-------------------------------------------------------------------------------}

-- | Block metadata
--
-- Models the Bcc BlockMeta, replacing the map key types:
-- * replace Bcc\TxId with DSL\TxId
-- * replace Bcc\Address with DSL\Addr
data BlockMeta' h = BlockMeta' {
      -- | SlotIds for all confirmed transactions
      _blockMetaSlotId'      :: Map (h (DSL.Transaction h Addr)) SlotId

    , -- | Address metadata using the DSL `Addr` and the Bcc `AddressMeta`
      _blockMetaAddressMeta' :: Map Addr AddressMeta
    }

-- | Interpretation of block metadata
--
-- NOTE: BlockMeta' has a mix of DSL and Bcc values, only the DSL values are translated
instance DSL.Hash h Addr => Interpret DSL2Bcc h (BlockMeta' h) where
  type Interpreted DSL2Bcc (BlockMeta' h) = BlockMeta
  ...
  int (BlockMeta' txs' addrMeta') = do
      ...
      where
          ...
          intAddrMeta (addr,addrMeta) = (,addrMeta) . InDb . addrInfoBcc <$> (int @DSL2Bcc) addr
...
instance DSL.Hash h Addr => Interpret DSL2Bcc h (DSL.Transaction h Addr) where
  type Interpreted DSL2Bcc (DSL.Transaction h Addr) = RawResolvedTx

  int :: forall e m. Monad m
      => DSL.Transaction h Addr -> IntT h e m RawResolvedTx
  int t = do
      let currentTime = getSomeTimestamp -- Pos.Core.Timestamp $ minBound
      (trIns', resolvedInputs) <- unzip <$> mapM (int @DSL2Bcc) (DSL.trIns' t)
      trOuts'                  <-           mapM (int @DSL2Bcc) (DSL.trOuts t)
...
instance DSL.Hash h Addr => Interpret DSL2Bcc h (DSL.Block h Addr) where
  -- The block and the EBB, if any
  type Interpreted DSL2Bcc (DSL.Block h Addr) = (RawResolvedBlock, Maybe GenesisBlock)

  int :: forall e m. Monad m
      => DSL.Block h Addr -> IntT h e m (RawResolvedBlock, Maybe GenesisBlock)
  int (OldestFirst txs) = do
      (txs', resolvedTxInputs) <- unpack <$> mapM (int @DSL2Bcc) txs
...
instance DSL.Hash h Addr => Interpret DSL2Bcc h (DSL.Chain h Addr) where
  type Interpreted DSL2Bcc (DSL.Chain h Addr) = OldestFirst [] Block

  int :: forall e m. Monad m
      => DSL.Chain h Addr -> IntT h e m (OldestFirst [] Block)
  int (OldestFirst blocks) =
      OldestFirst . concatMap flatten <$> mapM (int @DSL2Bcc) blocks
    where
...
instance Interpret DSL2Bcc h IntRollback where
  type Interpreted DSL2Bcc IntRollback = ()
...
instance DSL.Hash h Addr => Interpret DSL2Bcc h (IntSwitchToFork h) where
  type Interpreted DSL2Bcc (IntSwitchToFork h) = OldestFirst [] RawResolvedBlock

  int (IntSwitchToFork 0 (OldestFirst blocks)) = do
      OldestFirst . map fst <$> mapM (int @DSL2Bcc) blocks
  int (IntSwitchToFork n bs) = do
      int @DSL2Bcc $ IntRollback
      int @DSL2Bcc $ IntSwitchToFork (n - 1) bs
