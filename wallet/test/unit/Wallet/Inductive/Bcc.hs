...
module Wallet.Inductive.Bcc (
    -- * Bcc interpreter for the inductive wallet
    EventCallbacks(..)
  , interpretT
    -- * Equivalence check
...
import qualified Bcc.Wallet.Kernel.DB.AcidState as DB
import qualified Bcc.Wallet.Kernel.DB.BlockContext as DB
import qualified Bcc.Wallet.Kernel.DB.HdWallet as HD
import           Bcc.Wallet.Kernel.DB.InDb (InDb (..), fromDb)
import qualified Bcc.Wallet.Kernel.DB.Resolved as DB
import qualified Bcc.Wallet.Kernel.Internal as Internal
import           Bcc.Wallet.Kernel.Invariants as Kernel
import qualified Bcc.Wallet.Kernel.Keystore as Keystore
import qualified Bcc.Wallet.Kernel.Pending as Kernel
import           Bcc.Wallet.Kernel.PrefilterTx (prefilterUtxo)
import qualified Bcc.Wallet.Kernel.Read as Kernel
import           Bcc.Wallet.Kernel.Transactions (toMeta)
...
{-------------------------------------------------------------------------------
  Interpreter for the wallet using the translated Bcc types
-------------------------------------------------------------------------------}
...
-- | Interpreter for inductive wallets using the translated Bcc types
--
...
interpretT useWW injErr mkWallet EventCallbacks{..} Inductive{..} =
  ...
  where
    ...
    int' :: Interpret DSL2Bcc h a
         => History
         -> IntCtxt h
         -> a
         -> TranslateT e m (Interpreted DSL2Bcc a, IntCtxt h)
    int' hist ic =
        mapTranslateErrors (injErr hist . pretty) . runIntT' ic . int @DSL2Bcc
  ...
  where
    ...
    walletBootT ctxt utxo = do
        ...
        where
            ...
            -- as a Bcc Wallet with exactly one Account.
    ...
    checkWalletState ctxt@InductiveCtxt{..} accountId = do
      ...
      where
        cmp :: ( Interpret DSL2Bcc h a
               , Eq (Interpreted DSL2Bcc a)
               , Buildable a
               , Buildable (Interpreted DSL2Bcc a)
               , Buildable err
               )
            => Text
            -> (Wallet h Addr -> a)
            -> Either err (Interpreted DSL2Bcc a)
            -> TranslateT EquivalenceViolation m ()
        cmp fld _ (Left err) =
    ...
    toBcc :: Interpret DSL2Bcc h a
              => InductiveCtxt h
              -> Text
              -> a -> TranslateT EquivalenceViolation m (Interpreted DSL2Bcc a)
    toBcc InductiveCtxt{..} fld a = do
        ma' <- catchTranslateErrors $ runIntT' inductiveCtxtInt $ int @DSL2Bcc a
        case ma' of
          Left err -> throwError
...
data EquivalenceViolation =
    -- | Bcc wallet and pure wallet are not equivalent
    EquivalenceViolation {
...
data EquivalenceViolationEvidence =
    forall a. (Buildable a, Buildable (Interpreted DSL2Bcc a)) => NotEquivalent {
        notEquivalentDsl        :: a
      , notEquivalentTranslated :: Interpreted DSL2Bcc a
      , notEquivalentKernel     :: Interpreted DSL2Bcc a
      }
