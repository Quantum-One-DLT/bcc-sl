...
import           UTxO.Crypto
import           UTxO.DSL
import           UTxO.ToBcc.Interpreter (BlockMeta' (..), DSL2Bcc,
...
specBody pm = do
  ...
  where
    ...
    ourActorIx   = 0
    model        = (bccModel linearFeePolicy ourActorIx (transCtxtAddrs transCtxt) boot)
    ...
    intBlockMeta :: forall h. (Interpret DSL2Bcc h (BlockMeta' h))
                 => IntCtxt h
                 -> (BlockMeta' h)
                 -> TranslateT IntException IO BlockMeta
    intBlockMeta intCtxt a = do
        ma' <- catchTranslateErrors $ runIntT' intCtxt $ (int @DSL2Bcc) a
        case ma' of
