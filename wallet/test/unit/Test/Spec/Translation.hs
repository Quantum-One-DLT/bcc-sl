...
specBody pm = do
    describe "Translation sanity checks" $ do
      ...
      prop "can construct and verify chain that spans epochs" $ withMaxSuccess 5 $
        let epochSlots = runTranslateNoErrors pm $ asks (ccEpochSlots . tcBcc)
    ...
    describe "Translation QuickCheck tests" $ do
      prop "can translate randomly generated chains" $ withMaxSuccess 5 $
        forAll
          (intAndVerifyGen pm (genChainUsingModel . bccModel linearFeePolicy ourActorIx allAddrs))
          expectValid
...
intAndVerifyChain pm pc = runTranslateT pm $ do
    ...
    intResult <- catchTranslateErrors $ runIntBoot' boot $ int @DSL2Bcc chain
    case intResult of
      Left e ->
        case dslIsValid of
      ...
      Right (chain', ctxt) -> do
        let chain'' = fromMaybe (error "intAndVerify: Nothing")
                    $ nonEmptyOldestFirst
                    $ chain'
        isBccValid <- verifyBlocksPrefix Original chain'' dummyTxValRules
        case (dslIsValid, isBccValid) of
          ...
          (Valid     () , Valid (_undo, finalUtxo)) -> do
            (finalUtxo', _) <- runIntT' ctxt $ int @DSL2Bcc dslUtxo
...
data Disagreement h a =
  ...
  | UnexpectedValid Text
    ...
    -- a different UTxO
    -- UnexpectedUtxo utxoDsl utxoBcc utxoInt
...
instance (Hash h a, Buildable a) => Buildable (Disagreement h a) where
  ...
  build (UnexpectedUtxo utxoDsl utxoBcc utxoInt) = bprint
      ( "UnexpectedUtxo"
      % "{ dsl:     " % build
      % ", bcc: " % mapJson
      % ", int:     " % mapJson
