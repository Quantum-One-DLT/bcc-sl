...
class ToEntropics a where
    toEntropics :: a -> Word64

instance ToEntropics InitialBalance where
    toEntropics (InitialEntropic w) = w
    toEntropics (InitialBCC w)      = w * 1000000

instance ToEntropics Pay where
    toEntropics (PayEntropic w) = w
    toEntropics (PayBCC w)      = w * 1000000
...
-- | Generate an Utxo. The returned Utxo is not valid from a Bcc
...
genFiddlyUtxo payment = do
    let balance         = toEntropics payment
...
genUtxoWithAtLeast payment = do
    ...
    genUtxo $ StakeGenOptions {
                ...
                , stakeNeeded           = Core.mkCoin (toEntropics payment)
...
genGroupedUtxo groups balance = do
    let toHave = toEntropics balance
...
genRedeemPayee _utxo payment = do
    ...
    return (Core.TxOut a (Core.mkCoin (toEntropics payment)) :| [])
