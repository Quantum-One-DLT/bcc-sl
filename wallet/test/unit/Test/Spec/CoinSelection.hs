...
import           Bcc.Wallet.Kernel.CoinSelection (CoinSelFinalResult (..),
                     CoinSelHardErr (..), CoinSelPolicy,
                     CoinSelectionOptions (..), ExpenseRegulation (..),
                     InputGrouping (..), estimateMaxTxInputsExplicitBounds,
                     largestFirst, newOptions, random)
import           Bcc.Wallet.Kernel.CoinSelection.FromGeneric (Bcc,
                     estimateBccFee, estimateHardMaxTxInputsExplicitBounds)
import           Bcc.Wallet.Kernel.Transactions (mkStdTx)
import           Bcc.Wallet.Kernel.Util.Core (paymentAmount, utxoBalance,
...
minFee _ outs = Core.mkCoin (fromIntegral $ NE.length outs)

-- | An hopefully-accurate estimate of the Tx fees in Bcc.
bccFee :: Int -> NonEmpty Core.Coin -> Core.Coin
bccFee inputs outputs = Core.mkCoin $
    estimateBccFee linearFeePolicy inputs (toList $ fmap Core.getCoin outputs)
    where
...
genMaxInputTx :: ProtocolMagic -> (Byte -> Byte -> Byte -> Word64) -> Gen (Either Text (Byte, Byte))
genMaxInputTx pm estimator = do
...
type Policy = CoinSelectionOptions
           -> Word64
           -> CoinSelPolicy Core.Utxo Gen (CoinSelFinalResult Bcc)
...
payRestrictInputsTo :: ProtocolMagic
                    -> Word64
                    ...
                    -> Policy
                    -> Gen RunResult
payRestrictInputsTo pm maxInputs genU genP feeFunction adjustOptions bal amount policy =
    withProvidedMagicConfig pm $ \genesisConfig _ _ -> do
        utxo  <- genU bal
...
pay :: ProtocolMagic
    ...
    -> Policy
    -> Gen RunResult
pay pm = payRestrictInputsTo pm maxNumInputs

payOne :: ProtocolMagic
       -> (Int -> NonEmpty Core.Coin -> Core.Coin)
       -> (CoinSelectionOptions -> CoinSelectionOptions)
       -> InitialBalance
       -> Pay
       -> Policy
       -> Gen RunResult
payOne pm = pay pm genUtxoWithAtLeast genPayee
...
payOne' :: ProtocolMagic
        ...
        -> Policy
        -> Gen RunResult
payOne' pm payeeGenerator = pay pm genUtxoWithAtLeast payeeGenerator

payBatch :: ProtocolMagic
         -> (Int -> NonEmpty Core.Coin -> Core.Coin)
         -> (CoinSelectionOptions -> CoinSelectionOptions)
         -> InitialBalance
         -> Pay
         -> Policy
         -> Gen RunResult
payBatch pm = pay pm genUtxoWithAtLeast genPayees
    ...
    describe "Coin selection policies unit tests" $ do
        withMaxSuccess 5 $ describe "largestFirst" $ do
            prop "one payee, SenderPaysFee, fee = 0" $ \pm -> forAll (
                 payOne pm freeLunch identity (InitialEntropic 1000) (PayEntropic 100) largestFirst
                 ) $ \(utxo, payee, res) -> paymentSucceeded utxo payee res
            prop "one payee, ReceiverPaysFee, fee = 0" $ \pm -> forAll (
                 payOne pm freeLunch receiverPays (InitialEntropic 1000) (PayEntropic 100) largestFirst
                 ) $ \(utxo, payee, res) -> paymentSucceeded utxo payee res
            prop "multiple payees, SenderPaysFee, fee = 0" $ \pm -> forAll (
                 payBatch pm freeLunch identity (InitialEntropic 1000) (PayEntropic 100) largestFirst
                 ) $ \(utxo, payee, res) -> paymentSucceeded utxo payee res
            prop "multiple payees, ReceiverPaysFee, fee = 0" $ \pm -> forAll (
                 payBatch pm freeLunch receiverPays (InitialEntropic 1000) (PayEntropic 100) largestFirst
                 ) $ \(utxo, payee, res) -> paymentSucceeded utxo payee res

            -- Minimal fee
            prop "one payee, SenderPaysFee, fee = 1 Entropic" $ \pm -> forAll (
                payOne pm minFee identity (InitialEntropic 1000) (PayEntropic 100) largestFirst
                ) $ \(utxo, payee, res) -> paymentSucceeded utxo payee res
            prop "one payee, ReceiverPaysFee, fee = 1 Entropic" $ \pm -> forAll (
                payOne pm minFee receiverPays (InitialEntropic 1000) (PayEntropic 100) largestFirst
                ) $ \(utxo, payee, res) -> paymentSucceeded utxo payee res
            prop "multiple payees, SenderPaysFee, fee = 1 Entropic" $ \pm -> forAll (
                payBatch pm minFee identity (InitialEntropic 1000) (PayEntropic 100) largestFirst
                ) $ \(utxo, payee, res) -> paymentSucceeded utxo payee res
            prop "multiple payees, ReceiverPaysFee, fee = 1 Entropic" $ \pm -> forAll (
                payBatch pm minFee receiverPays (InitialEntropic 1000) (PayEntropic 100) largestFirst
                ) $ \(utxo, payee, res) -> paymentSucceeded utxo payee res

        withMaxSuccess 5 $ describe "random" $ do
            prop "one payee, SenderPaysFee, fee = 0" $ \pm -> forAll (
                payOne pm freeLunch identity (InitialEntropic 1000) (PayEntropic 100) random
                ) $ \(utxo, payee, res) -> paymentSucceeded utxo payee res
            prop "one payee, ReceiverPaysFee, fee = 0" $ \pm -> forAll (
                payOne pm freeLunch receiverPays (InitialEntropic 1000) (PayEntropic 100) random
                ) $ \(utxo, payee, res) -> paymentSucceeded utxo payee res
            prop "multiple payees, SenderPaysFee, fee = 0" $ \pm -> forAll (
                payBatch pm freeLunch identity (InitialEntropic 1000) (PayEntropic 100) random
                ) $ \(utxo, payee, res) -> paymentSucceeded utxo payee res
            prop "multiple payees, ReceiverPaysFee, fee = 0" $ \pm -> forAll (
                payBatch pm freeLunch receiverPays (InitialEntropic 1000) (PayEntropic 100) random
                ) $ \(utxo, payee, res) -> paymentSucceeded utxo payee res
            ...
            prop "one payee, SenderPaysFee, fee = 1 Entropic" $ \pm -> forAll (
                payOne pm minFee identity (InitialEntropic 1000) (PayEntropic 100) random
                ) $ \(utxo, payee, res) ->
                  paymentSucceededWith utxo payee res [feeWasPayed SenderPaysFee]
            prop "multiple payees, SenderPaysFee, fee = 1 Entropic" $ \pm -> forAll (
                payBatch pm minFee identity (InitialEntropic 1000) (PayEntropic 100) random
                ) $ \(utxo, payee, res) ->
                  paymentSucceededWith utxo payee res [feeWasPayed SenderPaysFee]

            -- linear fee
            prop "one payee, ReceiverPaysFee, fee = linear" $ \pm -> forAll (
                payOne pm linearFee receiverPays (InitialEntropic 1000) (PayEntropic 100) random
                ) $ \(utxo, payee, res) ->
                  paymentSucceededWith utxo payee res [feeWasPayed ReceiverPaysFee]
            prop "multiple payees, ReceiverPaysFee, fee = linear" $ \pm -> forAll (
                payBatch pm linearFee receiverPays (InitialEntropic 1000) (PayEntropic 100) random
                ) $ \(utxo, payee, res) ->
                  paymentSucceededWith utxo payee res [feeWasPayed ReceiverPaysFee]

            -- bcc fee. In order for this policy to succeed, we need to
            -- have some non-negligible amount of coins in our initial Utxo, as
            -- `estimateBccFee` works on "real world estimates"  for things
            -- like attributes, and trying to setup syntetic experiments with
            -- less than 1BCC (10^6 entropics) is probably counter-productive
            prop "one payee, SenderPaysFee, fee = bcc" $ \pm -> forAll (
                payOne pm bccFee identity (InitialBCC 1000) (PayBCC 100) random
                ) $ \(utxo, payee, res) ->
                  paymentSucceededWith utxo payee res [feeWasPayed SenderPaysFee]
            prop "multiple payees, SenderPaysFee, fee = bcc" $ \pm -> forAll (
                pay pm genFragmentedUtxo genPayees bccFee identity (InitialBCC 1000) (PayBCC 100) random
                ) $ \(utxo, payee, res) ->
                  paymentSucceededWith utxo payee res [feeWasPayed SenderPaysFee]
            prop "one payee, ReceiverPaysFee, fee = bcc" $ \pm -> forAll (
                payOne pm bccFee receiverPays (InitialBCC 1000) (PayBCC 100) random
                ) $ \(utxo, payee, res) ->
                  paymentSucceededWith utxo payee res [feeWasPayed ReceiverPaysFee]
            prop "multiple payees, ReceiverPaysFee, fee = bcc" $ \pm -> forAll (
                payBatch pm bccFee receiverPays (InitialBCC 1000) (PayBCC 100) random
                ) $ \(utxo, payee, res) ->
                  paymentSucceededWith utxo payee res [feeWasPayed ReceiverPaysFee]

        withMaxSuccess 5 $ describe "Expected failures" $ do
            prop "Paying a redeem address should always be rejected" $ \pm -> forAll (
                payOne' pm genRedeemPayee linearFee receiverPays (InitialEntropic 1000) (PayEntropic 100) random
                ) $ \(utxo, payee, res) ->
            ...
            prop "Paying somebody not having enough money should fail" $ \pm -> forAll (
                payBatch pm linearFee receiverPays (InitialEntropic 10) (PayEntropic 100) random
                ) $ \(utxo, payee, res) -> do
            ...
            prop "Restricting too much the number of inputs results in an hard error for a single payee" $ \pm -> forAll (
                payRestrictInputsTo pm 1 genUtxoWithAtLeast genPayee freeLunch identity (InitialEntropic 200) (PayEntropic 100) random
                ) $ \(utxo, payee, res) -> do
            ...
            prop "Restricting too much the number of inputs results in an hard error for multiple payees" $ \pm -> forAll (
                payRestrictInputsTo pm 1 genUtxoWithAtLeast genPayees freeLunch identity (InitialEntropic 200) (PayEntropic 100) random
                ) $ \(utxo, payee, res) -> do
                  paymentFailedWith utxo payee res [errorWas maxInputsReached]

        -- Tests that the coin selection is unaffected by the size of the
        -- Addresses in Bcc, as in the past there was a subtle corner case
        -- where coin selection would fail for Addresses of size < 104, which is
        -- the average in Bcc.
        withMaxSuccess 5 $ describe "Fiddly Addresses" $ do
            prop "multiple payees, SenderPaysFee, fee = bcc" $ \pm -> forAll (
                pay pm genFiddlyUtxo genFiddlyPayees bccFee identity (InitialBCC 1000) (PayBCC 100) random
                ) $ \(utxo, payee, res) ->
                  paymentSucceededWith utxo payee res [feeWasPayed SenderPaysFee]
            prop "multiple payees, ReceiverPaysFee, fee = bcc" $ \pm -> forAll (
                pay pm genFiddlyUtxo genFiddlyPayees bccFee receiverPays (InitialBCC 1000) (PayBCC 100) random
                ) $ \(utxo, payee, res) ->
                  paymentSucceededWith utxo payee res [feeWasPayed ReceiverPaysFee]
        ...
        withMaxSuccess 5 $ describe "Input Grouping" $ do
            prop "Require grouping, fee = 0, one big group depletes the Utxo completely" $ \pm -> forAll (
                pay pm (genGroupedUtxo 1) genPayee freeLunch requireGrouping (InitialEntropic 1000) (PayEntropic 10) random
                ) $ \(utxo, payee, res) -> do
                  paymentSucceededWith utxo payee res [utxoWasDepleted]
            prop "Require grouping, fee = bcc, one big group depletes the Utxo completely" $ \pm -> forAll (
                pay pm (genGroupedUtxo 1) genPayee freeLunch requireGrouping (InitialBCC 1000) (PayBCC 10) random
                ) $ \(utxo, payee, res) -> do
                  paymentSucceededWith utxo payee res [utxoWasDepleted]
            prop "Require grouping, fee = 0, several groups allows the payment to be fullfilled" $ \pm -> forAll (
                pay pm (genGroupedUtxo 10) genPayee freeLunch requireGrouping (InitialEntropic 1000) (PayEntropic 10) random
                ) $ \(utxo, payee, res) -> do
                  paymentSucceeded utxo payee res
            prop "Prefer grouping, fee = 0" $ \pm -> forAll (
                payOne pm freeLunch preferGrouping (InitialEntropic 1000) (PayEntropic 10) random
                ) $ \(utxo, payee, res) -> do
                  paymentSucceeded utxo payee res
            prop "IgnoreGrouping, fee = 0 must not deplete the utxo" $ \pm -> forAll (
                pay pm (genGroupedUtxo 1) genPayee freeLunch ignoreGrouping (InitialEntropic 1000) (PayEntropic 10) random
                ) $ \(utxo, payee, res) -> do
