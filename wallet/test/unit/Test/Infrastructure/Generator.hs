...
--  derive all of these arguments. See 'simpleModel' and 'bccModel'.
...
-- | The Bcc itself (given the bootstrap transaction).
--
-- This is a model that results in something that we can translate to Bcc,
...
bccModel linearFeePolicy ourActor allAddrs boot = GeneratorModel {
      gmBoot          = boot
