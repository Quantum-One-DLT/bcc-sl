...
import           Bcc.Cluster (MaxWaitingTime (..), NodeName (..),
                     NodeType (..), startNode)
import           Bcc.Cluster.Environment (Artifact (..), Env,
                     prepareEnvironment, withSystemStart)
import           Bcc.Cluster.Util (execParserEnv, oneSecond, runAsync,
                     stripFilterPrefix, varFromParser)
import           Bcc.Wallet.Action (actionWithWallet)
import           Bcc.Wallet.Client.Http (ClientError (..), Manager,
                     ServantError (..), WalletClient (getNodeInfo),
                     WalletHttpClient)
import           Bcc.Wallet.Server.CLI (walletBackendParamsParser)
...
-- This is a temporary, simplified version of what in 'Bcc.Cluster' that
