...
spec = do
    ...
    scenario "REG CO-449: extra guard for too big transactions" $ do
        fixtureSource <- setup $ defaultSetup
            & initialCoins .~ [oneBcc, oneBcc, oneBcc, oneBcc, oneBcc]
    ...
    scenario "REG CO-450: fallback works correctly" $ do
        fixtureSource <- setup $ defaultSetup
            & initialCoins .~ [oneBcc, oneBcc, oneBcc, oneBcc]
        fixtureDest <- setup $ defaultSetup
        accountDest <- successfulRequest $ Client.getAccount
            $- (fixtureDest ^. wallet . walletId)
            $- defaultAccountId
    ...
    scenario "REG CO-450: coin selection error reports correct balance" $ do
        fixtureSource <- setup $ defaultSetup
            & initialCoins .~ [oneBcc, oneBcc, oneBcc]
        fixtureDest <- setup $ defaultSetup
        ...
        verify resp
            [ expectWalletError $
                NotEnoughMoney $ ErrAvailableBalanceIsInsufficient $ 3*oneBcc
            ]
    ...
    scenario "balance is increased when valid key is redemeed" $ do
        ...
        sequence_
            [ do -- // 1 Redeem a certificate
                response <- request $ Client.redeemBcc $- Redemption
