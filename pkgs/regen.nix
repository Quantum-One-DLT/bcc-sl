...
, config ? {}
, tbcoPkgs ? import ../. { inherit config system; }
, pkgs ? tbcoPkgs.pkgs
...
let
  nix-tools
    = import
        ...
        { inherit pkgs; };
  deps = [ nixStable coreutils git findutils cabal2nix glibcLocales stack cabal-install tbcoPkgs.stack2nix
