...
-- | Translation of an abstract chain into Bcc.
--
...
module Chain.Abstract.Translate.ToBcc
  (
...
import           Bcc.Wallet.Kernel.Types (RawResolvedBlock,
...
{-------------------------------------------------------------------------------
  Translate the Abstract chain definitions to Bcc types
-------------------------------------------------------------------------------}

data Abstract2Bcc

instance Interpretation Abstract2Bcc where
  type IntCtx Abstract2Bcc = IntT

{-------------------------------------------------------------------------------
  Instances that read, but not update, the state
-------------------------------------------------------------------------------}

instance Interpret Abstract2Bcc h DSL.Value where
  type Interpreted Abstract2Bcc DSL.Value = Coin

  int :: Monad m => DSL.Value -> IntT h e m Coin
  int = return . mkCoin

instance Interpret Abstract2Bcc h Addr where
  type Interpreted Abstract2Bcc Addr = AddrInfo

  int :: Monad m => Addr -> IntT h e m AddrInfo
  int = asks . resolveAddr

instance Interpret Abstract2Bcc h (Output h Addr) where
  type Interpreted Abstract2Bcc (Output h Addr) = TxOutAux

  int :: Monad m
      => Output h Addr
      -> IntT h e m TxOutAux
  int Output{..} = do
    AddrInfo{..} <- (int @Abstract2Bcc) outAddr
    outVal'      <- (int @Abstract2Bcc) outVal
    return TxOutAux {
      toaOut = TxOut {
...
instance DSL.Hash h Addr => Interpret Abstract2Bcc h (DSL.Input h Addr) where
  type Interpreted Abstract2Bcc (DSL.Input h Addr) =
  ...
  int inp@DSL.Input{..} = do
      ...
      resolvedInput <- (int @Abstract2Bcc) spentOutput
      AddrInfo{..}  <- (int @Abstract2Bcc) $ outAddr spentOutput
...
instance DSL.Hash h Addr => Interpret Abstract2Bcc h (Transaction h Addr) where
  type Interpreted Abstract2Bcc (Transaction h Addr) = RawResolvedTx

  int :: forall e m. Monad m
      => Transaction h Addr -> IntT h e m RawResolvedTx
  int t = do
      (trIns', resolvedInputs) <- unzip . toList <$> mapM (int @Abstract2Bcc) (trIns  t)
      trOuts'                  <-         toList <$> mapM (int @Abstract2Bcc) (trOuts t)
...
instance DSL.Hash h Addr => Interpret Abstract2Bcc h (Block h Addr) where
  -- The block and the EBB, if any
  type Interpreted Abstract2Bcc (Block h Addr) = (RawResolvedBlock, Maybe GenesisBlock)

  int :: forall e m. Monad m
      => Block h Addr -> IntT h e m (RawResolvedBlock, Maybe GenesisBlock)
  int b = do
      let (OldestFirst txs) = blockTransactions b
      (txs', resolvedTxInputs) <- unpack <$> mapM (int @Abstract2Bcc) txs
...
instance DSL.Hash h Addr => Interpret Abstract2Bcc h (Chain h Addr) where
  type Interpreted Abstract2Bcc (Chain h Addr) = OldestFirst [] PosChain.Block

  int :: forall e m. Monad m
      => Chain h Addr -> IntT h e m (OldestFirst [] PosChain.Block)
  int (OldestFirst blocks) =
      OldestFirst . concatMap flatten <$> mapM (int @Abstract2Bcc) blocks
    where
...
instance Interpret Abstract2Bcc h IntRollback where
  type Interpreted Abstract2Bcc IntRollback = ()
