...
verifyBlock genesisConfig verifyAllIsKnown (ComponentBlockMain header payload) = do
    ...
    execRollT $ do
        ...
        recordBlockIssuance
            ...
            (header ^. headerHashG)
  where
    sbftProcessGenesisBlock ::
        ( MonadPoll m
        , MonadError PollVerFailure m
        )
        => EpochIndex -> m BlockVersion
    sbftProcessGenesisBlock currentEpoch = do
        logDebug $ "usVerifyBlocks SBFT: We're on epoch boundary. Running processGenesisBlock."
        processGenesisBlock genesisConfig currentEpoch
        getAdoptedBV
