...
import           Pos.Chain.Update (BlockVersion (..), ConsensusEra (..),
                     SbftConsensusStrictness (..), UpdateConfiguration,
    ...
    leaders <- case era of
        ...
        SBFT SbftStrict -> do
            initialSlot <- case curSlot of
                                Just cs -> pure cs
                                Nothing -> throwError "slogVerifyBlocks SbftStrict: curSlot set to Nothing - \
                                            \this occurs in EBBs which should not appear"
            pure $ SbftStrictLeaders $
                getEpochSlotLeaderScheduleSbft genesisConfig
                                               (siEpoch initialSlot)
        SBFT SbftLenient -> do
            -- The lenient SBFT block validation algorithm only requires a
            ...
            pure $
                SbftLenientLeaders (Set.fromList gStakeholders)
