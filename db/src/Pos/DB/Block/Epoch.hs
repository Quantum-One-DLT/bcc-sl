...
-- This function is designed to work on both Shardagnostic classic (Original)
-- epochs and on Shardagnostic BFT epochs. The only difference between these two
-- epoch types from the point of view of block consolidation is that Original
-- epochs start with an epoch boundary block (EBB) where as SBFT doesn't have
-- EBBs.
-- The inital header hash that is passed in should be the hash of either the
-- EBB for Original or of the zeroth block in the case of SBFT.
getEpochHeaderHashes
    :: MonadDBRead m
    => HeaderHash -> ExceptT ConsolidateError m (HeaderHash, [SlotIndexHash])
getEpochHeaderHashes startHash = do
    -- Make sure the hash passed to the SBFT version is a Main block and not an
...
data ConsolidateCheckPoint = ConsolidateCheckPoint
    ...
    , ccpHeaderHash :: !HeaderHash
      ...
      -- Shardagnostic Original/Classic, this will be the epoch boundary block
      -- and in the case of SBFT, this with be the zeroth block of the next
      -- epoch.
    }
