...
import Explorer.Util.String (formatBCC, substitute, parseSearchEpoch)
    ...
    describe "Explorer.Util.String" do
        ...
        describe "query parser" do
            ...
            it "parses an epoch and a slot and raises error" do
                ...
                result `shouldEqual` expected
        describe "formatBCC" do
            it "formats big number of entropics using EN format" do
                let result = formatBCC (mkCoin "123456789123456789") English
                result `shouldEqual` "123,456,789,123.456789"
            it "formats zero entropics using EN format" do
                let result = formatBCC (mkCoin "0") English
                result `shouldEqual` "0.000000"
            it "formats big number of entropics using DE format" do
                let result = formatBCC (mkCoin "123456789123456789") German
                result `shouldEqual` "123.456.789.123,456789"
            it "formats zero entropics using DE format" do
                let result = formatBCC (mkCoin "0") German
                result `shouldEqual` "0,000000"
            it "formats big number of entropics using JP format" do
                let result = formatBCC (mkCoin "123456789123456789") Japanese
                result `shouldEqual` "123,456,789,123.456789"
            it "formats zero entropics using JP format" do
                let result = formatBCC (mkCoin "0") Japanese
                result `shouldEqual` "0.000000"
