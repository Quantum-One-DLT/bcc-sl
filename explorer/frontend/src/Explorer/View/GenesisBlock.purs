...
-- | Detail page of a `genesis block` https://bccdocs.com/technical/blocks/#genesis-block
...
import Explorer.Util.String (formatBCC)
...
import Pos.Explorer.Web.Lenses.ClientTypes (_CAddress, cgaiBccAddress, cgaiGenesisAmount, cgaiIsRedeemed, cgsNonRedeemedAmountTotal, cgsNumNotRedeemed, cgsNumRedeemed, cgsNumTotal, cgsRedeemedAmountTotal, getCoin)
...
mkPropsForSummaryRows lang (CGenesisSummary summary) =
    ...
    , { key: 3
      ...
      , amount: formatBCC (summary ^. cgsRedeemedAmountTotal) lang
      , mCurrency: Just BCC
      }
    , { key: 4
      , label: translate (I18nL.genesisBlock <<< I18nL.gblNonRedeemedAmountTotal) lang
      , amount: formatBCC (summary ^. cgsNonRedeemedAmountTotal) lang
      , mCurrency: Just BCC
      }
    ]
...
addressInfosBodyView lang (CGenesisAddressInfo info) =
    let addrLink = toUrl $ Address (info ^. cgaiBccAddress)
        addrString = info ^. (cgaiBccAddress <<< _CAddress)
    in
    S.div ! S.className "address-infos__body--row"
          ...
          S.div ! S.className "address-infos__body--column amount"
                $ S.span ! S.className (currencyCSSClass $ Just BCC)
                          $ S.text (formatBCC (info ^. cgaiGenesisAmount) lang)
          -- is redeemed
