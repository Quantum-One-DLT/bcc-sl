...
let
  ...
  bowerComponents = buildBowerComponents {
    name = "bcc-sl-explorer-frontend-deps";
  ...
  regen-script = writeScriptBin "regen" ''
    export PATH=${makeBinPath [oldHaskellPackages.purescript-derive-lenses bcc-sl-explorer]}:$PATH
    bcc-explorer-hs2purs --bridge-path src/Generated/
    scripts/generate-explorer-lenses.sh
  '';

  frontend = { stdenv, python, mkYarnPackage }:
    mkYarnPackage {
      name = "bcc-explorer-frontend";
      inherit src;
      yarnLock = ./yarn.lock;
  ...
  # See also: bcc-sl/scripts/set-git-rev/default.nix
