...
, stateDir ? maybeEnv "BCC_STATE_DIR" "./state-acceptance-test-${environment}"
, environment ? "mainnet"
, resume ? true
}:
...
in
  writeScript "acceptance-tests-${environment}" ''
    ...
    hp2pretty bcc-node.hp

    if [ -n "$BUILDKITE" ]; then
      buildkite-agent artifact upload bcc-node.svg
      printf '\033]1338;url='"artifact://bcc-node.svg"';alt='"Heap profile"'\a\n'
    fi
