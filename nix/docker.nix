...
let
  ...
  startScriptConnect = name: args: writeScriptBin "bcc-start-${name}" ''
    #!/bin/sh
    set -euo pipefail
  ...
  startScripts = [
    ...
    (writeScriptBin "bcc-start-node" ''
      ...
      cd /state
      node_args="--db-path /state/db --rebuild-db --listen 0.0.0.0:3000 --json-log /state/logs/node.json --logs-prefix /state/logs --log-config /state/logs/log-config-node.yaml --metrics +RTS -N2 -qg -A1m -I0 -T -RTS --configuration-file ${bccConfig}/lib/configuration.yaml --configuration-key ${confKey}"

      exec ${bcc-node-simple}/bin/bcc-node-simple $node_args "$@"
    '')
  ];
...
in dockerTools.buildImage {
  ...
  contents = [ iana-etc openssl bashInteractive coreutils utillinux iproute iputils curl socat ]
   ++ startScripts;
  config = {
    Cmd = [ "bcc-start-${if (type != null) then type else "wallet"}" ];
    ExposedPorts = {
      "3000/tcp" = {}; # protocol
      "8090/tcp" = {}; # wallet
      "8091/tcp" = {}; # wallet doc
