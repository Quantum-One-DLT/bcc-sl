...
, bcc-node, bcc-explorer, bcc-x509-certificates
, bccConfig

## options!
, environment ? "mainnet"
, stateDir ? maybeEnv "BCC_STATE_DIR" "state-${executable}-${environment}"
, executable ? "wallet"
, topologyFile ? null
...
let
  ...
  executables =  {
    wallet = "${bcc-node}/bin/bcc-node";
    explorer = "${bcc-explorer}/bin/bcc-explorer";
    x509gen = "${bcc-x509-certificates}/bin/bcc-x509-certificates";
  };
  ...
  configurationArgs = concatStringsSep " " [
    "--configuration-file ${env.confFile or "${bccConfig}/lib/configuration.yaml"}"
  ...
  tlsConfigResultant = {
    ...
    serverCommonName = "Bcc SL Server Node";
...
in writeScript "${executable}-connect-to-${environment}" ''
  ...
  exec ${executables.${executable}}                                                    \
    ...
    --log-config ${bccConfig}/log-configs/connect-to-cluster.yaml              \
