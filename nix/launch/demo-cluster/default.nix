...
, bcc-sl-cluster-demo, bccConfig, bcc-node, bcc-sl-cluster-prepare-environment

## lots of options!
, stateDir ? maybeEnv "BCC_STATE_DIR" "./state-demo"
, runWallet ? true
, runExplorer ? false
, numCoreNodes ? 4
, numRelayNodes ? 1
...
let
  bccDeps = [ bcc-sl-cluster-demo bcc-node bcc-sl-cluster-prepare-environment ];
  demoClusterDeps = [ jq coreutils curl gnused openssl ];
  allDeps =  demoClusterDeps ++ bccDeps;
...
in writeScript "demo-cluster" ''
  ...
  export DEMO_CONFIGURATION_FILE=${bccConfig}/lib/configuration.yaml
  ...
  # to force the `stop_bcc` function to exit with a different code.
  EXIT_STATUS=0
  function stop_bcc {
    trap "" INT TERM
    echo "Received TERM!"
    echo "Stopping Bcc Demo Cluster"
    kill $pidCluster
    kill $pidWallet
    echo "Stopped all Bcc processes, exiting with code $EXIT_STATUS!"
    exit $EXIT_STATUS
  }
  ...
  trap "stop_bcc" INT TERM
  echo "Launching a demo cluster..."
  echo "Configuration file: $DEMO_CONFIGURATION_FILE"
  bcc-sl-cluster-prepare-environment "DEMO_" --cores ${builtins.toString numCoreNodes} --relays ${builtins.toString numRelayNodes} --edges ${builtins.toString numEdgeNodes}
  bcc-sl-cluster-demo --cores ${builtins.toString numCoreNodes} --relays ${builtins.toString numRelayNodes} --edges 0 &
  pidCluster=$!
