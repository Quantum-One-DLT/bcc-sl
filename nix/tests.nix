...
let
  pkgs = commonLib.pkgs;
  bccWallet = nixTools.nix-tools.cexes.bcc-wallet.bcc-generate-swagger-file;
...
in {
  ...
  swaggerSchemaValidation = pkgs.callPackage ./tests/swaggerSchemaValidation.nix { inherit bccWallet validateJson; };
}
