...
-- | This is a magic value which signifies that we are in the SBFT era.
-- Since we are no longer decentralizing this codebase, we can repurpose
-- this field to signify the shift to SBFT.
sbftEraFlagValue :: EpochIndex
sbftEraFlagValue = EpochIndex 9999999999999999999

data SbftConsensusStrictness = SbftStrict | SbftLenient
    deriving (Eq, Show, Generic)

instance NFData SbftConsensusStrictness

data ConsensusEra = Original | SBFT SbftConsensusStrictness
    deriving (Eq, Show, Generic)

instance NFData ConsensusEra

-- | This function uses the repurposed field `bvdUnlockStakeEpoch` to
-- tell us whether we are in the Original or SBFT consensus eras.
consensusEraBVD :: BlockVersionData -> ConsensusEra
consensusEraBVD bvd = if sbftEraFlagValue == bvdUnlockStakeEpoch bvd
                         then SBFT SbftLenient
                         else Original
