...
verifyTxUtxo protocolMagic ctx@VTxContext {..} lockedAssets ta@(TxAux UnsafeTx {..} witnesses) = do
  ...
  where
    ...
    -- all Bcc are secured by single cryptographic signatures. In addition, before
    -- decentralisation, all blocks are being mined/minted by TBCO, The Blockchain Co and the Bcc
    -- Foundation. These machines are run by the TBCO devops team and all run the same version
    -- of the software. Before decentralisation, the lockedAssets can provide extra protection for
    -- the Bcc held by these entities. It is intended that this lockedAssets functionality will be
