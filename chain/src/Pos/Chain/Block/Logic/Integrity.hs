    ...
    relatedToLeaders leaders =
        case h of
            ...
            BlockHeaderMain mainHeader ->
                ...
                in case leaders of
                    -- For the `SBFT SbftLenient` era, we only check whether the
                    ...
                    -- a slot leader schedule as it would for the `SBFT SbftStrict`
                    -- and `Original` cases.
                    SbftLenientLeaders ldrs blkSecurityParam lastBlkSlots ->
                        [   ( blockSlotLeader `elem` ldrs
                            , sformat ("SbftLenient: slot leader who published block, "%build%", is not an acceptable leader.")
                                    blockSlotLeader
                              )
                        ,   ( sbftLeaderCanMint blockSlotLeader blkSecurityParam lastBlkSlots
                            , sformat ("SbftLenient: slot leader who published block, "%build%", has minted too many blocks ("% build %") in the past "%build%" slots.")
                                    blockSlotLeader
                    ...
                    SbftStrictLeaders ldrs ->
                        if isNothing (scheduleSlotLeader ldrs)
                            then [ (isJust (scheduleSlotLeader ldrs), "SbftStrict: scheduled slot leader is missing") ]
                            else
                                [   ( Just blockSlotLeader == scheduleSlotLeader ldrs
                                    , sformat ("SbftStrict: slot leader from schedule, "%build%", is different from slot leader who published block, "%build%".")
      ...
      where
        ...
        sbftLeaderCanMint :: AddressHash PublicKey -> BlockCount -> LastBlkSlots -> Bool
        sbftLeaderCanMint leaderAddrHash blkSecurityParam lastBlkSlots =
