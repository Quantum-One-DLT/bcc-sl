...
-- unique so that block vaidation doesn't fail when validating SbftLenient.
...
genHeaderAndParams pm era = do
    ...
    -- If the blkSecurityParam is too low (ie < 10) then SbftLenient is likely
    -- to fail.
    ...
    let (prev, header) =
        ...
        params = Block.VerifyHeaderParams
            ...
            , Block.vhpLeaders = case era of
                ...
                SBFT SbftStrict  -> SbftStrictLeaders <$> thisEpochLeaderSchedule
                SBFT SbftLenient ->
                    pure $ SbftLenientLeaders
...
mkEpochLeaderSchedule era eos hdrs =
  ...
  where
    ...
    -- boundary block) but at 0 for SbftStrict.
    extra :: Int
    extra =
        case era of
            Original -> 1
            ...
            SBFT SbftLenient ->
                ...
                Prelude.error "Test.Pos.Chain.Block.Arbitrary.mkEpochLeaderSchedule SbftLenient"
